...
#define topDisjointPathsSet 4
#define maxSizeOfDisjointSet 5
...
vector<vector<disjointPathsS>> populateDisjointPathSets(allPathsS paths){
	...
	int size;
	int count[maxSizeOfDisjointSet];
	for (int i = 0; i < maxSizeOfDisjointSet; i++)
		count[i] = 0;
	int cond = 0;
	bool status;
	vector<disjointPathsS> temp;
//	cout << "size in at FPDPS " << size << endl;
	for (int i = 0; i < paths.disjointPaths.size(); i++)
	{
		status = true;
		for (int j = 0; j < maxSizeOfDisjointSet; j++){
			if (count[i] != topDisjointPathsSet)
				status = false;
		}
		if(status)
			break;
		...
		size = paths.disjointPaths[i].disjointPaths.size();
		if((size < maxSizeOfDisjointSet) && (count[size-1] < topDisjointPathsSet)){
			count[size-1] += 1;;
			temp.push_back(paths.disjointPaths[i]);
		}
	}
