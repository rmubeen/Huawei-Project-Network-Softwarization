...
vector<vector<int>> findDisjointPaths(paths pathsVar){
	// making paths superset
	int totPaths = pathsVar.pathVec.size();
	vector<int> pathsSet(totPaths);
	for(int i =0; i < totPaths; i++){
		pathsSet[i] = pathsVar.pathVec[i].id;
	}
	vector<vector<int>> pathsSuperSet = subsets(pathsSet);

	// making disjoint table
	vector<vector<int>> disjointTable(totPaths);
	for(int i =0; i < totPaths; i++){
		disjointTable[i].resize(totPaths);
	}
	for(int i =0; i < totPaths; i++){
	...
	// filtering paths superset
	int i = 0;
	int size = pathsSuperSet.size();
	int increment = 0;
	bool status;
	...
	while(i < size){
		...
		for(int j = 0; j < pathsSuperSet[i].size(); j++){
			for(int k = j+1; k < pathsSuperSet[i].size(); k++){
				if(disjointTable[pathsSuperSet[i][j]][pathsSuperSet[i][k]] == 0){
					status = true;
					break;
				}
			}
		...
		if(status){
			increment = 0;
			size -= 1;
			pathsSuperSet.erase(pathsSuperSet.begin() + i);
		} else {
			sort(pathsSuperSet[i].begin(), pathsSuperSet[i].end());
		}
		i += increment;
	}

	pathsSuperSet = removeRepitionAndSingles(pathsSuperSet);
	sort(pathsSuperSet.begin(), pathsSuperSet.end());

	return pathsSuperSet;
}
